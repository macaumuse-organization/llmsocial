...
- 对外请求走不走代理由「设置 → 网络代理」决定：`src/server/proxy.ts` 的 `applyProxy()` 换掉 undici 的全局 dispatcher，所有连接器和模型 SDK 都用 Node 自带的 `fetch`，下一个请求就生效，不用重启。新写的对外请求一律用全局 `fetch`（连接器里用 `ctx.fetch`），别自己建 dispatcher、`http.request` 或别的 HTTP 客户端——会绕开这个开关。`NODE_USE_ENV_PROXY` 不用：它只在进程启动那一刻读一次，做不成开关。
