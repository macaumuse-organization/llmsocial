...
import type { Material, PlatformId, Skill } from '../shared/types.ts';
...
export function seed(repos: Repos, skillsDir: string): void {
  ...
  if (repos.settings.getRaw('seededExamples') === null) {
    ...
    const base = { successCriteria: '', linkFallback: '', materials: [] as Material[], allowedPlatforms: [] as PlatformId[], personaId: null, providerIds: [] as string[], mode: 'copilot' as const, maxDays: 7, maxTurns: 20, replyDelayMinS: 8, replyDelayMaxS: 25, followupEnabled: false, followupAfterH: 20, followupMax: 1, enabled: true };
    ...
    repos.campaigns.create({ ...base, name: '示例：分享一条短视频', goalType: 'share_content', goal: '让对方去看指定的那条视频，并回来说说看法。', successCriteria: '对方说看了，并给了一句具体的反馈。', facts: '〔视频讲什么、在哪个平台、标题是什么。〕', allowedLinks: [], linkFallback: '我主页置顶那条', skillIds: ids('rapport-basics', 'language-matching', 'content-share', 'material-match') });
