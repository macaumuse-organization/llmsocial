...
const NOISE = [
  /^\d{1,2}:\d{2}$/, // 10:25
  /^(上午|下午|凌晨|晚上|中午|早上)\s*\d{1,2}:\d{2}$/,
  /^(昨天|今天|前天|星期[一二三四五六日天]|周[一二三四五六日天])(\s*(上午|下午|凌晨|晚上)?\s*\d{1,2}:\d{2})?$/,
  /^\d{4}[年/-]\d{1,2}[月/-]\d{1,2}日?(\s+\d{1,2}:\d{2})?$/,
  /^\d{1,2}月\d{1,2}日(\s*(上午|下午|凌晨|晚上)?\s*\d{1,2}:\d{2})?$/,
...
export function parseChatScreenshot(lines: OcrLine[]): ParsedChatMessage[] {
  ...
  for (const line of kept) {
    ...
    if (prev && prev.side === side && line.y - prev.bottom < line.h * 0.9 && (side === 'me' || Math.abs(left - prev.left) < 0.04)) {
      const needsSpace = /[A-Za-z0-9,.!?]$/.test(prev.text) && /^[A-Za-z0-9]/.test(line.text);
      prev.text += (needsSpace ? ' ' : '') + line.text.trim();
